# Mac Storage Cleanup (Terminal Guide)

이 문서는 맥 컴퓨터(macOS)의 "시스템 데이터" 용량을 줄이기 위해 실제로 진행한 작업을 정리한 것입니다.
저는 Mac Mini 8GM 256GB 모델 사용중이며 주기적으로 스토리지 용량이 10GB미만으로 줄어드는 문제를 경험하고 있습니다. 
맥 스토리지 부족 문제를 해결하기 위해 터미널 기반으로 안전한 캐시/로그 정리와 개발 캐시 정리를 중심으로 진행했습니다.
